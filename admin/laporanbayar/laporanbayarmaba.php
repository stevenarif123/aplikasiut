...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['cari_mahasiswa'])) {
    ...
    $query = "SELECT * FROM mahasiswabaru WHERE NamaLengkap LIKE '%$nama_mahasiswa%' ORDER BY No DESC";
                ...
                if ($hasilPencarian && mysqli_num_rows($hasilPencarian) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($hasilPencarian)) {
                        echo '<tr>
                                  ...
                                  <td>' . $row['Jurusan'] . '</td>
                                  <td>
                                      <a href="penambahan.php?nim=' . '&nama=' . urlencode($row['NamaLengkap']) . '&jurusan=' . urlencode($row['Jurusan']) . '" class="btn btn-success">Tambah Laporan Bayar</a>
                                  </td>
                              </tr>';
                    }
                } else {
